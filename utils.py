...
def convert_to_two_hot(word):
    if(len(word) == 1 and word[0] in diacritics):
        ...
        letters_hot_vector = convert_word_to_one_hot(without_diacritics, is_letter=True)
        diacritics_hot_vector = convert_word_to_one_hot(only_diacritics, is_letter=False)
